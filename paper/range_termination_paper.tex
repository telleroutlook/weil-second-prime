...
%% ── Title ─────────────────────────────────────────────────────────────────────
\title[Range of a Finite-Scale Weil Positivity Method]{The Range of a Finite-Scale Weil Positivity Method:\\
       Success in the First Prime Window and Its Termination in the Second}

\author{Lin Tao}
\address{SAP China, 1717 Nanjing Road West, Shanghai 200040, China}
\email{teller.lin@outlook.com}

\subjclass[2020]{Primary 11M26; Secondary 65G20, 11M06}

\keywords{Weil quadratic form, explicit formula, finite-scale positivity,
  Schur complement, certified interval arithmetic, computer-assisted proof,
  range boundary}
...
Weil's criterion states \cite{Weil1952,Bombieri2000}:
\[
...
Recent independent studies of Weil's quadratic form from complementary
perspectives include \cite{Suzuki2026,Kim2026,Groskin2026}.
...
\cite{Tao2026WFP}.  This creates a sequence of intervals:
\begin{align*}
...
Following \cite{Tao2026WFP}, we work in a Legendre basis $\{P_n\}_{n \ge 0}$ on $L^2(-L,L)$, truncated
...
\begin{itemize}[leftmargin=2em]
  \item \certify\ --- outward-rounded interval arithmetic (Arb balls
    \cite{Johansson2017}); the
...
and is the one cited here \cite{Tao2026WFP}.
\end{remark}
...
\begin{tabular}{@{}lllp{.35\textwidth}l@{}}
\toprule
...
\begin{itemize}[leftmargin=2em]
  \item \textbf{Not a statement about RH.}  The method's range is a property
    ...
    height numerically \cite{PlattTrudgian2021}) are not the object under study.
    Failure of a sufficient criterion does not imply $\lambda(L) \le 0$.
...
\begin{enumerate}[leftmargin=2em,label=(\arabic*)]
  ...
  \item The adjudication methodology---outsourcing conjectures to independent
    ...
    contribution to the certification workflow \cite{Tao2026Proofctl}.
\end{enumerate}
...
\scriptsize
\begin{tabular}{@{}lp{.18\textheight}llp{.20\textheight}p{.13\textheight}p{.22\textheight}@{}}
\toprule
ID & Quantity & $L$ & Sector & Value / interval & Grade & Source \\
\midrule
P1  & $\lambda_{\mathrm{lb}}$ even                          & $7/20$        & even & $+7.81\times10^{-4}$                        & \certify            & W1: \path{lambda_profile.json} \\
P2  & $\lambda_{\mathrm{lb}}$ odd                           & $7/20$        & odd  & $+4.92\times10^{-2}$                        & \certify            & W1: \path{lambda_profile.json} \\
P3  & $c_L$                                                 & $7/20$        & —    & $1355726/993009$                            & exact               & W1: \path{cert_fp035_clean.json} \\
P4  & $\ltrue$ even, $d=60$ (kappa-corrected)                 & $0.350$       & even & $+2.6\times10^{-3}$                         & \pilot              & W1: kappa-corrected rescan (old \path{window_shape_even.checkpoint.json} superseded) \\
...
P7  & $\lambda_{\mathrm{lb}}$ vs.\ $\ltrue$ at $L{=}0.42$  & $0.42$        & even & $\lambda_{\mathrm{lb}}=9.31\times10^{-10}$; $\ltrue=+3.19\times10^{-5}$ & \pilot & W1: \path{lambda_profile.json} \\
P8  & $\max|\Delta C|$ cross                                & $3/5$         & even & $\ge 0.630$                                 & \certify            & W2: \path{s4_certify_cross_L060.json} \\
P9  & $\max|\Delta C|$ cross                                & $3/5$         & odd  & $\ge 0.140$                                 & \certify            & W2: \path{s4_certify_cross_L060.json} \\
P10 & min-pivot (full)                                      & $3/5$         & even & $[-0.01970,\,-0.01968]$                     & \certify            & W2: \path{s4_certify_cross_L060.json} \\
P11 & $\ltrue$ (trusted assembly)                           & $3/5$         & even & $-0.231$                                    & \pilot              & W2: \path{independent_dominance_anchor.json} \\
P12 & Layer-1 eig (refuted; reported $+0.077$, true $-0.231$) & $3/5$      & even & refuted                                     & \pilot\ refuted     & W2: \path{docs/OUTSOURCED_VERDICTS....md} \\
P13 & $\Delta V$ cross delay test                           & $0.552$       & even & $-2.744$                                    & \pilot              & W2: \path{docs/OUTSOURCED_VERDICTS....md} §3.2 \\
P14 & $\Delta V$ cross delay test                           & $0.580$       & even & $-0.403$                                    & \pilot              & W2: \path{docs/OUTSOURCED_VERDICTS....md} §3.2 \\
P15 & $\Delta V$ cross delay test                           & $0.600$       & even & $-2.251$                                    & \pilot              & W2: \path{docs/OUTSOURCED_VERDICTS....md} §3.2 \\
P16 & $\Delta_{\tau_3}$ (odd pivot contribution of $p{=}3$) & $3/5$         & odd  & $-0.113$                                    & \pilot              & W2: \path{s4_profile_L060.json} \\
P17 & $\Delta_{\mathrm{cross}}$ (even pivot contribution)   & $3/5$         & even & $+0.195$                                    & \pilot              & W2: \path{s4_profile_L060.json} \\
P18 & min-pivot odd                                         & $11/20$       & odd  & $[-2.815\times10^{-2},\,-1.720\times10^{-2}]$ & \certify          & W2: \path{s5_positivity_L055.json} \\
P19 & min-pivot even (indeterminate)                        & $11/20$       & even & $[-3.87\times10^{-4},\,+2.43\times10^{-4}]$& \certify            & W2: \path{s5_positivity_L055.json} \\
P20 & $\ltrue$ odd (certify anchor)                         & $69/100$      & odd  & $-0.36911$                                  & \certify\ anchor    & W2: \path{shape_certify_anchor_odd.json} \\
P21 & pivot straddle odd                                    & $69/100$      & odd  & $\approx -0.099$                            & \certify            & W2: \path{shape_certify_anchor_odd.json} \\
P22 & $\ltrue$ even (certify anchor)                        & $67/100$      & even & $-0.28596$                                  & \certify\ anchor    & W2: \path{shape_certify_anchor.json} \\
P23 & Odd pilot curve (9 pts, monotone)                     & window 2      & odd  & $-0.118 \to -0.369$                         & \pilot              & W2: \path{shape_odd_N7.json} \\
P24 & Even pilot curve (7 pts)                              & window 2      & even & $-0.093 \to -0.286$                         & \pilot              & W2: \path{shape_even_N8.json} \\
P25 & $d$-refinement (even basin not $d$-robust)            & $0.66$--$0.69$& even & artefact                                    & \pilot              & W2: \path{docs/SECOND_WINDOW_LAMBDA_SHAPE.md} §2 \\
P26 & $\Lambda_2$ window-2 right-end infimum                & ${\to}\log2^-$& odd  & $\approx -0.37$                             & \certify\ anchor    & W2: \path{shape_certify_anchor_odd.json} (=P20) \\
\bottomrule
\end{tabular}
\end{sidewaystable}
...
Comm.\ S\'em.\ Math.\ Univ.\ Lund, volume d\'edi\'e \`a Marcel Riesz (1952),
252--265.

\bibitem{Bombieri2000}
E.~Bombieri,
\emph{Remarks on {W}eil's quadratic functional in the theory of prime numbers,
{I}},
...
\emph{A numerical realization of {S}uzuki's {W}eil-quadratic-form operator:
the archimedean spectral law, its universality, and an operator form of
{W}eil's positivity criterion},
\arxiv{2607.24830} (2026).

\bibitem{Groskin2026}
A.~Groskin,
\emph{A finite {G}uinand--{W}eil dictionary and archimedean tail order for the
truncated {W}eil quadratic form},
...
\bibitem{Tao2026WFP}
L.~Tao,
\emph{Structural Lemmas for the First-Prime Window of the {W}eil Quadratic
Form},
preprint, Zenodo (2026),
\doi{10.5281/zenodo.21807498}.

\bibitem{Tao2026Proofctl}
L.~Tao,
\emph{When the Pilot Audits the Tool: Two Verification Blind Spots Exposed and
Closed},
preprint, Zenodo (2026),
\doi{10.5281/zenodo.21847941}.

\bibitem{PlattTrudgian2021}
D.~J.~Platt and T.~S.~Trudgian,
\emph{The {R}iemann hypothesis is true up to $3\times 10^{12}$},
